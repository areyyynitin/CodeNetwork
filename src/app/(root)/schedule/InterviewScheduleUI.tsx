...
function InterviewScheduleUI() {
  ...
  const { user } = useUser(); // Fixed spacing issue
  const [open, setOpen] = useState(false);
  const [isCreating, setIsCreating] = useState(false);

  const interviews = useQuery(api.interview.getAllInterviews) ?? [];
  const users = (useQuery(api.users.getUsers) ?? []) as {
    _id: Id<"users">;
...
const candidates = Array.isArray(users) ? users.filter((u) => u.role === "candidate") : [];
const interviewers = Array.isArray(users) ? users.filter((u) => u.role === "interviewer") : [];
